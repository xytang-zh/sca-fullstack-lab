...
| 3 | spring-cloud-monitor  | 8083 | 业务 | 服务器监控、TDengine、WebSocket 推送 |
...
## 9.2 spring-cloud-gateway（网关服务）

### 9.2.1 服务定位
统一 API 入口，所有 `/api/**` 请求先经过网关，做路由、鉴权、限流、跨域。

### 9.2.2 核心需求
1. 把 `/api/system/**` 转发到 spring-cloud-system，其他服务同理
...
### 9.2.6 数据模型
- 无业务表
- 路由配置存 Nacos（`spring-cloud-gateway.yaml`）
...
## 9.3 spring-cloud-auth（认证中心 / SSO Server）
...
### 9.3.7 服务间关系
- 上游：Gateway
- 下游：spring-cloud-system（查用户）、spring-cloud-message（发短信/邮件）
- 依赖：Redis、MongoDB（登录日志）、RabbitMQ（事件 `user.login`、`user.kickout`）

---

## 9.4 spring-cloud-system（系统管理服务）
...
## 9.5 spring-cloud-monitor（服务器监控服务）
...
| 存储模块 | TDengine 超级表 `spring-cloud_metrics`，按服务+实例标签 |
...
- MyBatis-Plus（业务表）
- Dubbo（调用 spring-cloud-system 查用户/部门）
- RabbitMQ（任务待办事件 → spring-cloud-message 推送）
- MinIO（审批附件）
...
### 9.6.7 服务间关系
- 上游：Gateway
- 调用：Dubbo 调 spring-cloud-system（用户/部门）
- 调用：HTTP 调 spring-cloud-file（上传附件）
- 发布事件：`task.todo` → spring-cloud-message 推送

---

## 9.7 spring-cloud-ai（AI 助手服务）
...
### 9.7.7 服务间关系
- 上游：Gateway
- 调用：HTTP 调 spring-cloud-file（可选，用 MinIO 直接 SDK）
- 发布事件：`doc.uploaded` → 异步向量化；`ai.message.created` → spring-cloud-search 索引

---

## 9.8 spring-cloud-message（消息中心服务）
...
## 9.9 spring-cloud-search（全文检索服务）
...
- MinIO：bucket `spring-cloud-files`、`spring-cloud-public`、`spring-cloud-temp`
...
## 9.12 spring-cloud-portal（公开门户服务）
...
- Spring Boot
- MyBatis-Plus（文章、新闻表）
- ElasticSearch（全文搜索调 spring-cloud-search）
- Redis（文章缓存、阅读量计数）
...
- 调用：调 spring-cloud-search 做全文搜索
- 依赖：MySQL、Redis、ES

---

## 9.13 spring-cloud-job（定时任务服务）
...
## 9.14 spring-cloud-report（低代码报表服务）
...
- 业务表：复用 spring-cloud-system、spring-cloud-monitor 的表
...
| 服务                    | 核心技术栈 | 数据库 |
|-----------------------|------------|--------|
| spring-cloud-gateway  | Spring Cloud Gateway + Sentinel + Sa-Token | 无 |
| spring-cloud-auth     | Sa-Token SSO + OAuth2 + tianai-captcha + Redisson | MySQL + Redis |
| spring-cloud-system   | MyBatis-Plus + dynamic-datasource + Dubbo + EasyExcel | MySQL + Redis |
| spring-cloud-monitor  | OSHI + TDengine + Netty + micrometer | TDengine + Redis |
| spring-cloud-workflow | Warm-Flow + Dubbo + RabbitMQ + MinIO | MySQL |
| spring-cloud-ai       | Spring AI + pgvector + MongoDB + MinIO + SSE | PostgreSQL + MongoDB + MinIO |
| spring-cloud-message  | RabbitMQ + Netty + Mail + SMS + Thymeleaf | MySQL + Redis |
| spring-cloud-search   | ElasticSearch + ik 分词 + RabbitMQ | ElasticSearch |
| spring-cloud-file     | MinIO + Redisson + KKFileView | MySQL + MinIO |
| spring-cloud-log      | Spring AOP + MongoDB + ShardingSphere 分表 | MongoDB |
| spring-cloud-portal   | MyBatis-Plus + ES + Redis + Sitemap | MySQL + Redis + ES |
| spring-cloud-job      | XXL-JOB Core + Dubbo | XXL-JOB Admin + MySQL |
| spring-cloud-report   | JimuReport + dynamic-datasource + Sa-Token | MySQL + 全部业务库（只读） |
...
| 优先级 | 服务                    | 依赖前置                                                       |
|--------|-----------------------|------------------------------------------------------------|
| P0 | spring-cloud-system   | 无（地基）                                                      |
| P0 | spring-cloud-auth     | spring-cloud-system                                        |
| P0 | spring-cloud-gateway  | spring-cloud-auth                                          |
| P1 | spring-cloud-file     | spring-cloud-auth                                          |
| P1 | spring-cloud-log      | RabbitMQ                                                   |
| P1 | spring-cloud-message  | RabbitMQ、spring-cloud-system                               |
| P2 | spring-cloud-monitor  | TDengine、spring-cloud-message                              |
| P2 | spring-cloud-workflow | spring-cloud-system、spring-cloud-message、spring-cloud-file |
| P2 | spring-cloud-portal   | spring-cloud-search                                        |
| P2 | spring-cloud-search   | ElasticSearch、RabbitMQ                                     |
| P3 | spring-cloud-ai       | pgvector、MongoDB、MinIO                                     |
| P3 | spring-cloud-job      | XXL-JOB Admin                                              |
| P3 | spring-cloud-report   | JimuReport、所有业务库                                           |
